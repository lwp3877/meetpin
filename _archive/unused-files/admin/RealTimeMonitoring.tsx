  ...
  const fetchLiveMetrics = useCallback(async () => {
    try {
      ...
      const [_onlineRes, roomsRes, reportsRes, signupsRes] = await Promise.all([
        ...
        supabase.from('profiles').select('count').gte('updated_at', fiveMinutesAgo),

        // 활성 방
        supabase.from('rooms').select('count').gte('start_at', new Date().toISOString()),
      ...
      // 봇 비율 계산
      const totalUsers = await supabase.from('profiles').select('count')

      const botUsers = await supabase.from('profiles').select('count').like('nickname', '%bot%') // 봇 식별 로직 개선 필요

      const botRatio = totalUsers.count ? ((botUsers.count || 0) / totalUsers.count) * 100 : 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <CardContent>
            <Badge className={getStatusColor(health?.dbStatus || 'error')}>
              {health?.dbStatus === 'healthy'
                ? '정상'
                : health?.dbStatus === 'warning'
                  ? '경고'
                  : '오류'}
            </Badge>
            {health && (
              <p className="mt-1 text-xs text-gray-500">응답시간: {health.responseTime}ms</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-primary text-2xl font-bold">{metrics?.currentOnline || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{metrics?.activeRooms || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{metrics?.pendingReports || 0}</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div>
              <h4 className="mb-2 font-medium">24시간 신규 가입</h4>
            ...
            <div>
              <h4 className="mb-2 font-medium">봇 비율</h4>
              <div className="text-2xl font-bold text-purple-600">{metrics?.botRatio || 0}%</div>
              <p className="text-xs text-gray-500">전체 사용자 대비 봇 비율</p>
            </div>

            <div>
              <h4 className="mb-2 font-medium">자동 새로고침</h4>
              <Button
              ...
              <p className="mt-1 text-xs text-gray-500">30초마다 자동 갱신</p>
            </div>
          </div>
        </CardContent>
      </Card>
