...
import { X, AlertTriangle, Shield, MessageSquare, User, MapPin } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl">
        {/* Header */}
        <div className={`bg-gradient-to-r ${getModalColor()} p-6 text-white`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                {targetName && <p className="text-sm opacity-90">대상: {targetName}</p>}
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="max-h-[calc(90vh-140px)] space-y-6 overflow-y-auto p-6">
            ...
            <div className="rounded-xl border border-red-200 bg-red-50 p-4">
              <div className="flex items-start space-x-2">
                ...
                <div className="text-sm text-red-800">
                  <p className="mb-1 font-medium">긴급 상황 신고</p>
                  <p className="text-xs">
          ...
          {/* 신고 유형 선택 */}
          <div>
            <h3 className="mb-3 font-medium text-gray-900">신고 유형</h3>
          ...
          {/* 신고 내용 */}
          <div>
            <h3 className="mb-2 font-medium text-gray-900">신고 내용</h3>
            <textarea
              value={description}
              onChange={e => setDescription(e.target.value)}
              placeholder="구체적인 신고 내용을 입력해주세요. (최소 10자)"
              className="h-32 w-full resize-none rounded-xl border border-gray-200 p-3 focus:border-transparent focus:ring-2 focus:ring-orange-500 focus:outline-none"
              maxLength={1000}
            />
            <div className="mt-1 text-xs text-gray-500">{description.length}/1000자</div>
          </div>

          {/* 긴급도 선택 (긴급 신고인 경우) */}
          {isEmergency && (
            <div>
              <h3 className="mb-3 font-medium text-gray-900">긴급도</h3>
              <div className="space-y-2">
          ...
          {isEmergency && (
            <div>
              <h3 className="mb-2 font-medium text-gray-900">위치 정보</h3>
              {currentLocation ? (
                <div className="rounded-xl border border-green-200 bg-green-50 p-3">
                  ...
                  {currentLocation.address && (
                    <p className="mt-1 text-xs text-green-700">{currentLocation.address}</p>
                  )}
                </div>
              ) : (
                <button
                  onClick={getLocation}
                  className="w-full rounded-xl border-2 border-dashed border-gray-300 p-3 text-gray-600 transition-colors hover:border-gray-400"
                >
          ...
          <div className="rounded-xl bg-gray-50 p-4">
            <div className="flex items-start space-x-2">
              ...
              <div className="text-sm text-gray-700">
                <p className="mb-1 font-medium">신고 관련 안내</p>
                <ul className="space-y-1 text-xs">
                  ...
                  <li>
                    •{' '}
                    {isEmergency
                      ? '긴급 신고는 24시간 내에 처리됩니다'
                      : '일반 신고는 2-3일 내에 처리됩니다'}
                  </li>
        ...
        <div className="flex space-x-3 border-t border-gray-100 p-6">
          ...
          <button
            ...
            className={`flex-2 bg-gradient-to-r px-6 py-3 ${getModalColor()} flex items-center justify-center space-x-2 rounded-xl font-medium text-white shadow-lg transition-all hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50`}
          >
            {submitting ? (
              <>
