    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">모임 커버 이미지</h3>
        </div>
        ...
        <div className="relative">
          {currentBackground ? (
            <div className="relative">
              {/* 실제 이미지나 그라디언트 미리보기 */}
              <div
                className={`${size === 'large' ? 'h-48' : 'h-32'} relative w-full overflow-hidden rounded-2xl`}
                style={{
                  background: selectedGradient || undefined,
                }}
              >
                {currentImageUrl && (
                  <Image src={currentImageUrl} alt="방 커버 이미지" fill className="object-cover" />
                )}

                {/* 오버레이 텍스트 */}
                <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                  <div className="text-center text-white">
                    <div className="mb-2 text-4xl">{categoryDisplay.emoji}</div>
                    <h2 className="text-xl font-bold drop-shadow-lg">{roomTitle}</h2>
                    <div className="mt-1 rounded-full bg-white/20 px-3 py-1 text-sm font-medium backdrop-blur-sm">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 flex h-8 w-8 items-center justify-center rounded-full bg-red-500 text-white shadow-lg transition-colors hover:bg-red-600"
                >
                  ×
                </button>
              </div>
      ...
      {showGradientOptions && (
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-semibold text-gray-900 dark:text-white">색상 테마 선택</h3>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {gradients.map((gradient, index) => (
              <button
                ...
                className={`h-16 rounded-xl transition-all duration-200 hover:scale-105 hover:shadow-lg ${selectedGradient === gradient ? 'ring-primary ring-4 ring-offset-2' : ''} `}
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
        <h4 className="mb-2 flex items-center font-semibold text-amber-900 dark:text-amber-100">
