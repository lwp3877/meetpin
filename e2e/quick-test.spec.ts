...
test.describe('간단한 배포 테스트', () => {
  test('배포된 사이트 기본 기능 확인', async ({ page }) => {
    ...
    const createRoomButton = await page.locator('button:has-text("방 만들기"), button:has-text("새 모임")').count() > 0
    ...
    const hasMockIndicator = pageContent.includes('Mock') || pageContent.includes('isDevelopmentMode')
