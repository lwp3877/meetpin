...
test.describe('밋핀 완전한 UI 테스트', () => {
  ...
  test('홈페이지 로드 및 기본 네비게이션 테스트', async ({ page }) => {
    ...
    const mapButton = page.locator('text=지도에서 모임 찾기').or(page.locator('text=지금 시작하기')).first()
  ...
  test('맵 페이지 기능 테스트', async ({ page }) => {
    ...
    // 맵 페이지 로드 대기
    await page.waitForSelector('[data-testid="map-container"], .map-container, [id*="map"], [class*="map"]', { timeout: 10000 }).catch(() => {})
  ...
  test('모바일 반응형 UI 테스트', async ({ page }) => {
    ...
    const mobileMenu = page.locator('[data-testid="mobile-menu"], .mobile-menu, button[aria-label*="메뉴"]').first()
