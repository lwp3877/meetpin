...
test.describe('업데이트된 샘플 데이터 검증', () => {
  ...
  test('다양한 지역의 방들이 존재하는지 API로 확인', async ({ page, request }) => {
    ...
    // 서울 전체 영역 API 호출
    const response = await request.get(`${BASE_URL}/api/rooms?bbox=37.4563,126.8226,37.6761,127.1836&limit=50`)
    ...
    // 프로필 이미지 확인
    const roomsWithImages = rooms.filter((room: any) => 
      room.profiles.avatar_url && room.profiles.avatar_url.includes('unsplash.com')
    )
  ...
  test('부스트된 방들이 우선 표시되는지 확인', async ({ page, request }) => {
    ...
    const response = await request.get(`${BASE_URL}/api/rooms?bbox=37.4563,126.8226,37.6761,127.1836&limit=10`)
    const data = await response.json()
