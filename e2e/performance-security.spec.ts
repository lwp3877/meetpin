...
test.describe('⚡ 성능 모니터링 자동화', () => {
  test('🏃‍♀️ 홈페이지 성능 벤치마크', async ({ page }) => {
    ...
    console.log('├─────────────────────────────────────┼─────────┼─────────┤')
    console.log(
      `│ 페이지 로드 시간 (ms)               │ ${metrics.loadTime.toFixed(0).padStart(7)} │ ${TEST_CONFIG.performanceThresholds.loadTime.toString().padStart(7)} │`
    )
    console.log(
      `│ DOM 로드 시간 (ms)                  │ ${metrics.domContentLoaded.toFixed(0).padStart(7)} │ ${TEST_CONFIG.performanceThresholds.domContentLoaded.toString().padStart(7)} │`
    )
    console.log(
      `│ First Contentful Paint (ms)         │ ${metrics.firstContentfulPaint.toFixed(0).padStart(7)} │ ${TEST_CONFIG.performanceThresholds.firstContentfulPaint.toString().padStart(7)} │`
    )
    console.log(
      `│ Largest Contentful Paint (ms)       │ ${metrics.largestContentfulPaint.toFixed(0).padStart(7)} │ ${TEST_CONFIG.performanceThresholds.largestContentfulPaint.toString().padStart(7)} │`
    )
    console.log(
      `│ Cumulative Layout Shift             │ ${metrics.cumulativeLayoutShift.toFixed(3).padStart(7)} │ ${TEST_CONFIG.performanceThresholds.cumulativeLayoutShift.toString().padStart(7)} │`
    )
    console.log(
      `│ Total Blocking Time (ms)            │ ${metrics.totalBlockingTime.toFixed(0).padStart(7)} │ ${TEST_CONFIG.performanceThresholds.totalBlockingTime.toString().padStart(7)} │`
    )
    console.log('└─────────────────────────────────────┴─────────┴─────────┘')

    // 성능 임계값 검증
    expect(metrics.loadTime, '페이지 로드 시간이 기준을 초과했습니다').toBeLessThan(
      TEST_CONFIG.performanceThresholds.loadTime
    )
    expect(metrics.domContentLoaded, 'DOM 로드 시간이 기준을 초과했습니다').toBeLessThan(
      TEST_CONFIG.performanceThresholds.domContentLoaded
    )

    if (metrics.firstContentfulPaint > 0) {
      expect(
        metrics.firstContentfulPaint,
        'First Contentful Paint가 기준을 초과했습니다'
      ).toBeLessThan(TEST_CONFIG.performanceThresholds.firstContentfulPaint)
    }

    if (metrics.largestContentfulPaint > 0) {
      expect(
        metrics.largestContentfulPaint,
        'Largest Contentful Paint가 기준을 초과했습니다'
      ).toBeLessThan(TEST_CONFIG.performanceThresholds.largestContentfulPaint)
    }

    expect(
      metrics.cumulativeLayoutShift,
      'Cumulative Layout Shift가 기준을 초과했습니다'
    ).toBeLessThan(TEST_CONFIG.performanceThresholds.cumulativeLayoutShift)
    expect(metrics.totalBlockingTime, 'Total Blocking Time이 기준을 초과했습니다').toBeLessThan(
      TEST_CONFIG.performanceThresholds.totalBlockingTime
    )
    ...
    if (resourceMetrics.totalSize > 5 * 1024 * 1024) {
      // 5MB
  ...
  test('📱 모바일 성능 검증', async ({ page, context }) => {
    ...
    // 모바일 성능은 더 관대한 기준 적용
    expect(mobileLoadTime, '모바일 로드 시간이 기준을 초과했습니다').toBeLessThan(
      TEST_CONFIG.performanceThresholds.loadTime * 2
    )
  ...
  test('🖼️ 이미지 최적화 검증', async ({ page }) => {
    ...
    const imageMetrics = await page.evaluate(() => {
      ...
      const results = images.map(img => ({
        ...
        isOptimized:
          img.src.includes('.webp') || img.src.includes('w_auto') || img.src.includes('q_auto'),
      }))
      ...
      return {
        ...
        oversizedImages: results.filter(
          r => r.naturalWidth > r.displayWidth * 2 || r.naturalHeight > r.displayHeight * 2
        ).length,
        details: results,
      }
    })
    ...
    console.log(
      `   - Alt 텍스트 있음: ${imageMetrics.imagesWithAlt}개 (${Math.round((imageMetrics.imagesWithAlt / imageMetrics.totalImages) * 100)}%)`
    )
    console.log(
      `   - 지연 로딩 적용: ${imageMetrics.lazyImages}개 (${Math.round((imageMetrics.lazyImages / imageMetrics.totalImages) * 100)}%)`
    )
    console.log(
      `   - 반응형 이미지: ${imageMetrics.responsiveImages}개 (${Math.round((imageMetrics.responsiveImages / imageMetrics.totalImages) * 100)}%)`
    )
    console.log(
      `   - 최적화된 포맷: ${imageMetrics.optimizedImages}개 (${Math.round((imageMetrics.optimizedImages / imageMetrics.totalImages) * 100)}%)`
    )
    console.log(`   - 크기 초과 이미지: ${imageMetrics.oversizedImages}개`)

    // 접근성 검증 (모든 이미지에 alt 속성)
    expect(imageMetrics.imagesWithAlt, '일부 이미지에 alt 텍스트가 없습니다').toBe(
...
test.describe('🔒 보안 취약점 자동 검증', () => {
  test('🛡️ XSS 공격 차단 검증', async ({ page }) => {
    ...
    // 다양한 XSS 페이로드 테스트
    for (const payload of TEST_CONFIG.securityTests.slice(0, 5)) {
      // 처음 5개만 테스트
  ...
  test('🔍 SQL Injection 차단 검증', async ({ page }) => {
    ...
    console.log(
      `🔍 SQL Injection 테스트 결과: ${errors.length > 0 ? '❌ 의심스러운 에러' : '✅ 안전'}`
    )
  ...
  test('🔐 인증 보안 검증', async ({ page }) => {
    ...
    const csrfToken = await page
      .locator('input[name="_token"], input[name="csrf_token"], meta[name="csrf-token"]')
      .first()
...
test.describe('♿ 접근성 자동 검증', () => {
  test('🔍 WCAG 2.1 AA 기준 검증', async ({ page }) => {
    ...
    const a11yResults = await page.evaluate(() => {
      ...
      inputs.forEach(input => {
        ...
        if (!label && !ariaLabel && !ariaLabelledby) {
          ...
          results.errors.push(
            `폼 요소에 레이블이 없습니다: ${input.tagName} ${(input as HTMLInputElement).type || ''}`
          )
        }
      })
    ...
    console.log('├─────────────────────────────────────┼─────────┤')
    console.log(
      `│ 이미지 (alt 없음)                   │ ${a11yResults.images} (${a11yResults.imagesWithoutAlt}) │`
    )
    console.log(`│ 헤딩 구조                           │ ${a11yResults.headings}개     │`)
    console.log(
      `│ 폼 요소 (레이블 없음)               │ ${a11yResults.forms} (${a11yResults.formsWithoutLabels})  │`
    )
    console.log(
      `│ 링크 (빈 링크)                      │ ${a11yResults.links} (${a11yResults.emptyLinks})   │`
    )
    ...
    const totalErrors =
      a11yResults.imagesWithoutAlt + a11yResults.formsWithoutLabels + a11yResults.emptyLinks
    const accessibilityScore =
      totalElements > 0 ? Math.round((1 - totalErrors / totalElements) * 100) : 100
  ...
  test('⌨️ 키보드 접근성 검증', async ({ page }) => {
    ...
    console.log(
      `   - 포커스 표시: ${elementsWithFocus}/${focusableElements.length} (${focusPercentage}%)`
    )
