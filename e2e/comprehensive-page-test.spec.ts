...
test.describe('🌟 완벽한 전체 페이지 테스트 시스템', () => {
  ...
  test('💬 핵심 기능 상호작용 테스트', async ({ page }) => {
    ...
    if (await messageButton.isVisible()) {
      ...
      // 모달 닫기
      const closeButton = page.locator('button:has-text("취소"), button[aria-label="닫기"], .close').first()
    ...
    // 지도 로딩 대기
    await page.waitForSelector('[class*="map"], #map, .leaflet-container', { timeout: 15000 }).catch(() => {
  ...
  test('🔍 검색 및 필터 기능 테스트', async ({ page }) => {
    ...
    const filterButtons = page.locator('button:has-text("술"), button:has-text("운동"), button:has-text("기타")')
