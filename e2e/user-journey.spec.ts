...
test.describe('🚀 밋핀 완전한 사용자 여정 테스트', () => {
  ...
  test('1️⃣ 홈페이지 접근 및 기본 기능 확인', async ({ page }) => {
    ...
    // 네비게이션 확인
    const mapButton = page.locator('a[href="/map"], button').filter({ hasText: /지도|모임/ }).first()
  ...
  test('2️⃣ 회원가입 플로우 완전 검증', async ({ page }) => {
    ...
    // 회원가입 버튼 클릭
    const submitButton = page.locator('button[type="submit"], button').filter({ hasText: /가입|회원가입|시작/ })
  ...
  test('3️⃣ 로그인 플로우 완전 검증', async ({ page }) => {
    ...
    const loginButton = page.locator('button[type="submit"], button').filter({ hasText: /로그인|시작/ })
    ...
    // 사용자 정보 표시 확인
    const userIndicator = page.locator('[data-testid="user-info"], .user-menu, [aria-label*="사용자"], .avatar')
  ...
  test('4️⃣ 지도 페이지 및 모임 검색 기능 검증', async ({ page, context }) => {
    ...
    await page.fill('input[type="email"]', TEST_CONFIG.testUser.email)
    await page.fill('input[type="password"]', TEST_CONFIG.testUser.password)
    await page.locator('button[type="submit"], button').filter({ hasText: /로그인/ }).click()
  ...
  test('5️⃣ 새 모임 생성 플로우 완전 검증', async ({ page, context }) => {
    ...
    // 모임 생성 페이지로 이동
    const createButton = page.locator('a[href="/room/new"], button').filter({ hasText: /만들기|생성|추가/ })
    ...
    if (await categorySelect.isVisible()) {
      ...
      const categoryButton = page.locator(`input[value="${TEST_CONFIG.testRoom.category}"], button`).filter({ hasText: /술|drink/ })
    ...
    // 모임 생성 버튼 클릭
    const createSubmitButton = page.locator('button[type="submit"], button').filter({ hasText: /생성|만들기|저장/ })
  ...
  test('6️⃣ 모임 참가 신청 및 매칭 플로우 검증', async ({ page, context }) => {
    ...
    if (await joinButton.isVisible()) {
      ...
      const successMessage = page.locator('.toast, .notification').filter({ hasText: /성공|완료|신청/ })
  ...
  test('7️⃣ 채팅 기능 완전 검증', async ({ page }) => {
    ...
    // 채팅 목록 페이지 접근 시도
    const chatMenuButton = page.locator('a[href*="chat"], a[href*="message"], button').filter({ hasText: /채팅|메시지/ })
    ...
    if (await chatMenuButton.isVisible()) {
      ...
      if (await chatList.isVisible()) {
        ...
        if (await firstChat.isVisible()) {
          ...
          // 채팅 인터페이스 확인
          const messageInput = page.locator('input[placeholder*="메시지"], textarea[placeholder*="메시지"], .message-input')
          ...
          if (await messageInput.isVisible() && await sendButton.isVisible()) {
            ...
            // 메시지 전송 확인
            await page.waitForTimeout(2000)
            const sentMessage = page.locator('.message, .chat-message').filter({ hasText: /자동화 테스트/ })
  ...
  test('8️⃣ 프로필 관리 기능 검증', async ({ page }) => {
    ...
    // 프로필 페이지 접근
    const profileButton = page.locator('a[href="/profile"], .user-menu, .avatar, button').filter({ hasText: /프로필|Profile/ })
      ...
      if (await editButton.isVisible()) {
        ...
        if (await nicknameInput.isVisible()) {
          ...
          if (await saveButton.isVisible()) {
            ...
            // 저장 성공 확인
            const successMessage = page.locator('.toast, .notification').filter({ hasText: /성공|완료|저장/ })
  ...
  test('📱 모바일 반응형 테스트', async ({ page }) => {
    ...
    for (const button of buttons.slice(0, 5)) { // 처음 5개 버튼만 체크
