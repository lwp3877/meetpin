...
test.describe('신규 사용자 전체 기능 테스트', () => {
  test('새 사용자 여정 - 회원가입부터 채팅까지', async ({ page }) => {
    ...
    const createRoomButton = page.locator('button:has-text("방 만들기"), button:has-text("새 모임")')
    ...
    // 방 만들기 버튼 클릭
    const newRoomButton = page.locator('button:has-text("방 만들기"), button:has-text("새 모임"), a:has-text("방 만들기")').first()
    ...
    if (await newRoomButton.count() > 0) {
      ...
      // 방 만들기 페이지나 모달 확인
      const hasRoomForm = await page.locator('input[name="title"], input[placeholder*="제목"]').count() > 0
    ...
    if (await profileLink.count() > 0) {
      ...
      // 프로필 관련 요소 확인
      const hasProfile = await page.locator('input[name="nickname"], .profile, h1:has-text("프로필")').count() > 0
