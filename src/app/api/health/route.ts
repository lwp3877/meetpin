...
async function checkServices() {
  ...
  try {
    ...
    if (supabaseUrl && supabaseAnonKey && supabaseServiceKey) {
      ...
      services.auth = (isValidUrl && isValidAnonKey && isValidServiceKey) 
        ? 'configured' 
        : 'missing_keys'
    } else {
  ...
  try {
    ...
    services.maps = kakaoKey && kakaoKey !== 'your-kakao-key' 
      ? 'configured' 
      : 'missing_key'
  } catch (_error) {
    services.maps = 'error'
  ...
  try {
    ...
    if (stripeSecretKey && stripePublishableKey && stripeWebhookSecret) {
      ...
      services.payments = (isValidSecretKey && isValidPublishableKey && isValidWebhookSecret)
        ? 'configured'
        : 'missing_keys'
    } else {
