...
const FORBIDDEN_WORDS = [
  '시발', '씨발', '개새끼', '좆', '병신', '미친새끼', '개놈', '년놈',
  '섹스', '성관계', '원나잇', '섹파', '폰섹', '조교', '강간', '성매매',
  '돈거래', '계좌이체', '송금', '대출', '투자', '도박', '베팅', '사기',
  '마약', '대마초', '필로폰', '히로뽕', '약물', '환각제'
];
...
async function getMessages(
  ...
  const { count: blockedCount } = await supabase
    ...
    .or(`and(blocker_uid.eq.${user.id},blocked_uid.eq.${otherUserId}),and(blocker_uid.eq.${otherUserId},blocked_uid.eq.${user.id})`)
