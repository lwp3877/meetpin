...
async function createRequest(request: NextRequest) {
  ...
  if ((currentCount || 0) >= (room as any).max_people - 1) {
    // 호스트 제외
  ...
  const { count: blockedCount } = await supabase
    ...
    .select('*', { count: 'exact', head: true })
    .or(
      `and(blocker_uid.eq.${user.id},blocked_uid.eq.${(room as any).host_uid}),and(blocker_uid.eq.${(room as any).host_uid},blocked_uid.eq.${user.id})`
    )
