...
import { Users, MapPin, MessageCircle, Heart, TrendingUp, Activity } from 'lucide-react'
...
export function LiveActivityStats() {
    ...
    const activitiesInterval = setInterval(() => {
      ...
      setRecentActivities(prev => [
        ...
        ...prev.slice(0, 4).map(activity => ({
          ...activity,
          time: activity.time === '방금 전' ? '1분 전' : 
                activity.time === '1분 전' ? '2분 전' :
                activity.time === '2분 전' ? '3분 전' :
                activity.time === '3분 전' ? '4분 전' : '5분 전'
        }))
      ])
    }, 6000)
  ...
  return (
    <div className="space-y-6">
      {/* Real-time Stats */}
      <Card className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-0 shadow-xl">
        <CardContent className="p-6">
          <div className="text-center mb-6">
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-2 flex items-center justify-center space-x-2">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {stats.map((stat) => (
              <div
                key={stat.id}
                className={`${stat.bgColor} p-4 rounded-xl text-center transition-all duration-300 hover:scale-105`}
              >
                ...
                <Badge 
                  variant="secondary" 
                  className={`text-xs ${
                    stat.trend === 'up' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' :
                ...
                >
                  <div className="flex items-center space-x-2">
                    <div className={`w-2 h-2 rounded-full ${
                      ...
                      activity.type === 'activity' ? 'bg-emerald-500' :
                      'bg-orange-500'
                    }`}></div>
      ...
      <Card className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border-purple-200 dark:border-purple-800 shadow-lg">
        <CardContent className="p-6">
          <h4 className="font-bold text-purple-700 dark:text-purple-300 mb-4 text-center">
            ⏰ 지금이 만남의 골든타임!
          </h4>
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
              <div className="text-lg font-bold text-purple-600 dark:text-purple-400">저녁 7-9시</div>
            ...
            <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
              <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">점심시간</div>
