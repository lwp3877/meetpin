...
export function LoadingCard({ 
  ...
  return (
    <Card className={`bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-white/20 dark:border-slate-700/30 shadow-2xl ${className}`}>
      <CardContent className="pt-12 pb-12 text-center">
        <div className="space-y-6">
          {/* 아이콘 또는 기본 스피너 */}
          <div className="relative">
            {icon ? (
              <div className="w-16 h-16 bg-gradient-to-br from-primary to-emerald-600 rounded-full mx-auto flex items-center justify-center shadow-2xl">
                {icon}
              </div>
            ) : (
...
export function NetworkLoadingCard() {
  return (
    <LoadingCard
      ...
      icon={<Wifi className="w-8 h-8 text-white animate-pulse" />}
    />
  )
}
...
export function RoomSearchLoadingCard() {
  return (
    <LoadingCard
      ...
      icon={<Search className="w-8 h-8 text-white animate-pulse" />}
    />
  )
}
...
export function ChatLoadingCard() {
  return (
    <LoadingCard
      ...
      icon={<MessageCircle className="w-8 h-8 text-white animate-pulse" />}
    />
  )
}
...
export function MatchingLoadingCard() {
  return (
    <LoadingCard
      ...
      icon={<Heart className="w-8 h-8 text-white animate-pulse" />}
    />
  )
}
...
export function Skeleton({ 
  ...
  return (
    <div 
      className={`bg-gray-200 dark:bg-gray-700 rounded ${variantClasses[variant]} ${animationClasses[animation]} ${className}`}
    />
  )
}
...
export function RoomCardSkeleton() {
  return (
    <Card className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border-white/20 dark:border-slate-700/30 shadow-xl">
  ...
  return (
    <div className={`text-center py-12 px-6 ${className}`}>
      <div className="space-y-6">
        {/* 아이콘 */}
        {icon && (
          <div className="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-full mx-auto flex items-center justify-center shadow-lg">
            {icon}
          </div>
        )}
    ...
    <EmptyState
      ...
      action={onCreateRoom && (
        <button
          ...
          className="px-6 py-3 bg-gradient-to-r from-primary to-emerald-600 text-white font-bold rounded-xl shadow-xl hover:shadow-2xl transition-all duration-200 hover:scale-105"
        >
  ...
  return (
    <EmptyState
      ...
      action={onRetry && (
        <button
          onClick={onRetry}
          className="px-6 py-3 bg-gradient-to-r from-red-500 to-orange-500 text-white font-bold rounded-xl shadow-xl hover:shadow-2xl transition-all duration-200 hover:scale-105"
        >
