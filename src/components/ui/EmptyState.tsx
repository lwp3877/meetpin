...
export default function EmptyState({
  ...
  return (
    <div className={`bg-white rounded-xl text-center border border-gray-100 ${sizeClass.container} ${className}`}>
      {icon && (
        <div className={`bg-gray-100 rounded-full mx-auto mb-6 flex items-center justify-center ${sizeClass.icon}`}>
          <span>{icon}</span>
        </div>
      )}
      
      <h3 className={`font-medium text-gray-900 mb-2 ${sizeClass.title}`}>
        {title}
      </h3>
      
      <p className={`text-gray-600 mb-6 max-w-md mx-auto leading-relaxed ${sizeClass.description}`}>
        {description}
      </p>
...
export const EmptyStatePresets = {
  ...
  NoActiveRooms: ({ onCreateRoom, onViewAll }: { onCreateRoom: () => void; onViewAll: () => void }) => (
    <EmptyState
      icon="🔥"
