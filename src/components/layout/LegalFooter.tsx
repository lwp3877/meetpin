...
export default function LegalFooter({ className = '', variant = 'default' }: LegalFooterProps) {
  const legalLinks = [
    {
      href: '/legal/privacy',
      label: '개인정보처리방침',
  ...
  return (
    <footer className={`${style.bg} ${className}`}>
      <div className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="lg:col-span-1">
            <div className="mb-4 flex items-center">
              ...
              <h3 className={`text-lg font-bold ${style.brandText}`}>{brandMessages.appName}</h3>
            </div>
            <p className={`text-sm ${style.text} leading-relaxed`}>{brandMessages.tagline}</p>
            <p className={`text-sm ${style.text} mt-2`}>새로운 사람들과의 특별한 만남</p>
          </div>

          {/* Legal links */}
          <div className="lg:col-span-3">
            <h4
              className={`text-sm font-semibold ${style.brandText} mb-4 tracking-wider uppercase`}
            >
              법무 정보
            </h4>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {legalLinks.map(link => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`group block ${style.linkText} transition-colors`}
                >
                  <div className="text-sm font-medium group-hover:underline">{link.label}</div>
                  <div className={`text-xs ${style.text} mt-1`}>{link.description}</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        {/* Legal disclaimer */}
        <div
          className={`mt-6 rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20`}
        >
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <p className="text-xs text-yellow-800 dark:text-yellow-200">
                <strong>⚖️ 법률 검토 필요:</strong>위 법무 문서들은 샘플 템플릿입니다. 실제 서비스
                운영 전 반드시 전문 법무팀의 검토를 받아야 합니다.
              </p>
            </div>
          </div>
        </div>
