...
describe('🔒 RLS Security Test Suite', () => {
  ...
  beforeAll(async () => {
    ...
    // 테스트 방 생성 (Alice가 호스트)
    const { data: roomData } = await adminClient.from('rooms').insert({
      title: 'RLS Test Room',
  ...
  describe('🏠 Room RLS Tests', () => {
    ...
    test('방 생성 시 host_uid는 현재 사용자와 일치해야 함', async () => {
      ...
      const { error: validCreateError } = await bobClient
        .from('rooms')
        .insert({
          title: 'Bob Room',
      ...
      const { error: invalidCreateError } = await bobClient
        .from('rooms')
        .insert({
  ...
  describe('🛡️ Admin RLS Tests', () => {
    ...
    test('관리자 권한 없이는 모든 데이터 조회 불가', async () => {
      ...
      // 모든 신고 내역 조회 시도 (실패)
      const { data: allReports } = await bobClient
        .from('reports')
        .select('*')
...
describe('📊 RLS Performance Tests', () => {
  ...
  test('대량 데이터에서 RLS 성능 확인', async () => {
    ...
    const { data: rooms, error } = await aliceClient
      .from('rooms')
      .select('*')
      .limit(1000)
